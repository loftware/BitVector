...
extension BitVector: Equatable {
  public static func == (l: Self, r: Self) -> Bool {
    ...
    let extraBits = l.count % UInt.bitWidth
    if extraBits != 0 {
      if !l.buckets.dropLast().elementsEqual(r.buckets.dropLast()) {
        return false
      }
      let l1 = l.buckets.last.unsafelyUnwrapped
      let r1 = r.buckets.last.unsafelyUnwrapped
      let differingBits = ~(l1 ^ r1)
      let mask = ((1 as UInt) << extraBits) &- 1
      return (differingBits & mask) == 0
    }
    return l.buckets.elementsEqual(r.buckets)
  }
}
